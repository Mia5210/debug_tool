...
class AmlAudioDebug:
    ...
    def start_capture(self, curTimeName, startCaptureFinish):
        ...
        self.__nowPullPcPath = AmlCommonUtils.get_path_by_module(self.__nowPullPcTime,
                                                                 AmlCommonUtils.AML_DEBUG_MODULE_AUDIO)
    ...
    def __capture_debug_text(self):
        ...
        dumpCmdListTemp = []
        for adbDumpCmd in self.__adbDumpCmdLists1:
        ...
        dumpCmdListTemp = []
        for adbDumpCmd in self.__adbDumpCmdLists2:
            dumpCmdListTemp.append('echo ' + adbDumpCmd + ' >> ' + self.__dumpCmdOutFilePath)
            dumpCmdListTemp.append(adbDumpCmd + ' >> ' + self.__dumpCmdOutFilePath)
        self.__exe_adb_shell_cmd(dumpCmdListTemp)
    ...
    def __capture_audio_data_start(self):
        ...
        self.log.d('2.1 AUTO mode: Start fetching the audio data, wait for ' + str(
            self.__debugCfg.m_autoDebugTimeS) + ' seconds...')
        if self.__debugCfg.m_captureMode == AmlAudioDebug.DEBUG_CAPTURE_MODE_AUTO:
            time.sleep(self.__debugCfg.m_autoDebugTimeS)
            self.__capture_audio_data_prop_disable()
    ...
    def __pull_capture_debug_info_to_pc(self):
        ...
        for dumpFile in self.__dumpFileLists:
            if (
                    self.__debugCfg.m_homeClick == True or self.__debugCfg.m_logcatEnable == False) and dumpFile == AmlCommonUtils.AML_DEBUG_PLATFORM_DIRECOTRY_LOGCAT or \
    ...
    def __capture_clear_all_files(self):
        if self.__debugCfg.m_homeClick == False:
            AmlCommonUtils.exe_adb_shell_cmd('rm ' + AmlCommonUtils.AML_DEBUG_PLATFORM_DIRECOTRY_LOGCAT + ' -rf',
    ...
    def start_play_toggle(self, filePath, channel, byte, rate, selChn, func_playEnd):
        ...
        if self.__m_isPlaying == False:
            thread = threading.Thread(target=self.__audio_pcm_play_thread,
                                      args=(filePath, channel, byte, rate, selChn, func_playEnd))
            thread.start()
        else:
    ...
    def __play_pcm_file(self, filePath):
        ...
        read_frames = 1024
        stream = amlPyAudio.open(format=amlPyAudio.get_format_from_width(bit_width), channels=channel, rate=sample_rate,
                                 output=True)
        self.log.i('__play_pcm_file starting to play' + ', frames:' + str(remain_frames) + ' (' + str(
            remain_frames // sample_rate) + ' s)')
