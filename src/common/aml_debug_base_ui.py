...
from src.common.aml_common_utils import *
...
class AmlDebugModule():
    ...
    def initModule(aml_ui):
        ...
        import src.Video.aml_debug_video_ui
        ...
        AmlDebugModule.moduleList.append(src.Audio.aml_debug_audio_ui.instance(aml_ui))#add audio module instance
        #AmlDebugModule.moduleList.append(src.Video.aml_debug_video_ui.instance(aml_ui))  # add audio module instance
...
class AmlDebugBaseUi(metaclass=ABCMeta):
    ...
    def __init__(self, mainUi, moduleId):
        ...
        self.log = log_print(mainUi.terminalLogSignal.emit, moduleId) #get log_print way(instance)
...
class log_print():
    ...
    def log(self, info, level=AmlCommonUtils.AML_DEBUG_LOG_LEVEL_D):
        if self.m_moduleId in AmlDebugBaseUi.AML_MODULE_NAME_ARRAY.keys():
            self.log_function('[' + AmlDebugBaseUi.AML_MODULE_NAME_ARRAY[self.m_moduleId] + ']' + info, level, self.m_moduleId)
        else:
            self.log_function('[Error] AmlDebugBaseUi can\'t find module id :' + self.m_moduleId, AmlCommonUtils.AML_DEBUG_LOG_LEVEL_E, self.m_moduleId)
